...
describe("periodIndexForDate", () => {
  ...
  it("puts a date at or before the closing day in that month's own invoice", () => {
    ...
    expect(
      periodIndexForDate(card, new Date("2026-07-05T00:00:00.000Z")),
    ).toEqual({
      year: 2026,
      month: 7,
    });
  ...
  it("includes the closing date itself (interval is end-inclusive)", () => {
    expect(
      periodIndexForDate(card, new Date("2026-07-10T00:00:00.000Z")),
    ).toEqual({
      year: 2026,
      month: 7,
    });
  ...
  it("pushes a date after the closing day into the next month's invoice", () => {
    ...
    expect(
      periodIndexForDate(card, new Date("2026-07-12T00:00:00.000Z")),
    ).toEqual({
      year: 2026,
      month: 8,
    });
  ...
  it("rolls December into January of the next year", () => {
    expect(
      periodIndexForDate(card, new Date("2026-12-20T00:00:00.000Z")),
    ).toEqual({
      year: 2027,
      month: 1,
    });
  ...
  it("clamps closingDay=31 to the last day of February", () => {
    ...
    expect(
      periodIndexForDate(card31, new Date("2026-02-28T00:00:00.000Z")),
    ).toEqual({
      year: 2026,
      month: 2,
    });
    ...
    expect(
      periodIndexForDate(card31, new Date("2026-03-01T00:00:00.000Z")),
    ).toEqual({
      year: 2026,
      month: 3,
    });
